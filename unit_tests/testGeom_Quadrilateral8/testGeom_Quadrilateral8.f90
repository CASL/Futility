...
SUBROUTINE testIntersectLine()
  ...
  CALL p7%init(DIM=2, X=0.0_SRK, Y=0.5_SRK)
  CALL p8%init(DIM=2, X=4.0_SRK, Y=0.5_SRK)
  CALL l%set(p7, p8)
  npoints = -1
  CALL intersect(quad, l, npoints, points)
  ASSERT(npoints == 3, "Intersects")
  ASSERT(points(1)%coord(1) .APPROXEQA. 1.5_SRK, "intersection 1")
  ASSERT(points(1)%coord(2) .APPROXEQA. 0.5_SRK, "intersection 1")
  ASSERT(points(2)%coord(1) .APPROXEQA. 41.0_SRK/18.0_SRK, "intersection 2")
  ASSERT(points(2)%coord(2) .APPROXEQA. 0.5_SRK, "intersection 2")
  ASSERT(points(3)%coord(1) .APPROXEQA. 0.0_SRK, "intersection 3")
  ASSERT(points(3)%coord(2) .APPROXEQA. 0.5_SRK, "intersection 3")
  CALL l%clear() 
  CALL p7%clear()
  CALL p8%clear()
  ...
  ASSERT(points(1)%coord(1) .APPROXEQA. 0.20557280900008415_SRK, "intersection 1")
  ASSERT(points(1)%coord(2) .APPROXEQA. 0.1_SRK, "intersection 1")
  ASSERT(points(2)%coord(1) .APPROXEQA. 1.9944271909999158_SRK, "intersection 2")
  ASSERT(points(2)%coord(2) .APPROXEQA. 0.1_SRK, "intersection 2")
  ASSERT(points(3)%coord(1) .APPROXEQA. 2.0644444444444447_SRK, "intersection 3")
  ASSERT(points(3)%coord(2) .APPROXEQA. 0.1_SRK, "intersection 3")
  ASSERT(points(4)%coord(1) .APPROXEQA. 0.0_SRK, "intersection 4")
