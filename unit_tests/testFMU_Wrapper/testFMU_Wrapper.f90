...
SUBROUTINE testStepFMU()
  ...
  CALL test_fmu2_slave%setNamedVariable("GRAVITY", grav_accl)
  CALL test_fmu2_slave%setNamedVariable("BOUNCE_COF", coeffRest)

  ! Set initial conditions
  CALL test_fmu2_slave%setNamedVariable("HEIGHT", 1.0_SRK)
  CALL test_fmu2_slave%setNamedVariable("HEIGHT_SPEED", 0.0_SRK)

  ! Check variable valueReferences
  ASSERT_EQ(test_fmu2_slave%getValueReference("HEIGHT"), 0_SIK, "%ballHeight_valueReference")
  CALL test_fmu2_slave%getReal(0_SIK, ballHeight)
  ASSERT_SOFTEQ(ballHeight, 1.0_SRK, 1.0E-12_SRK, "%ballHeight")
  ASSERT_EQ(test_fmu2_slave%getValueReference("HEIGHT_SPEED"), 1_SIK, "%ballVel_valueReference")
  CALL test_fmu2_slave%getReal(1_SIK, ballVel)
  ASSERT_SOFTEQ(ballVel, 0.0_SRK, 1.0E-12_SRK, "%ballVel")
  ...
  DO i=1,10
    ...
    time = time + dt
    CALL test_fmu2_slave%getNamedVariable("HEIGHT", ballHeight)
    CALL test_fmu2_slave%getNamedVariable("HEIGHT_SPEED", ballVel)
