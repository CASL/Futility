...
!>
    SUBROUTINE writeTable(funit,tablevals)
        ...
        DO k=1,maxrowsize(j)
          ...
          DO i=1,SIZE(tablevals,DIM=1)
            ...
            IF(k == 1) THEN
              ...
              !We're using the semi-colon+space so we have interoperability.
...
!>
    SUBROUTINE getTableBounds(tablevals,maxcolsize,maxrowsize)
      TYPE(StringType),INTENT(IN) :: tablevals(:,:)
      INTEGER(SIK),ALLOCATABLE,INTENT(OUT) :: maxcolsize(:)
      INTEGER(SIK),ALLOCATABLE,INTENT(OUT) :: maxrowsize(:)
