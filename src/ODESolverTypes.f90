...
! List of public members
  ...
  INTEGER(SIK),PARAMETER,PUBLIC :: ODE_NATIVE=1,ODE_SUNDIALS=2,ODE_RYTHMOS=3
  ...
  !> @brief the rythmos ode solver type
  TYPE,EXTENDS(ODESolverType_Base) :: ODESolverType_Rythmos
    !> map index of the ODE solver in trilinos
    INTEGER(SIK) :: id=-1
    !> order of BDF method
    INTEGER(SIK) :: BDForder=5
    !> temporary allocatable array
    REAL(C_DOUBLE),ALLOCATABLE :: ytmp0(:)
    REAL(C_DOUBLE),ALLOCATABLE :: ytmpf(:)
  !
  !List of Type Bound Procedures
    CONTAINS
      !> @copybrief ODESolverTypes::init_ODESolverType_Rythmos
      !> @copydetails ODESolverTypes::init_ODESolverType_Rythmos
      PROCEDURE,PASS :: init => init_ODESolverType_Rythmos
      !> @copybrief ODESolverTypes::clear_ODESolverType_Rythmos
      !> @copydetails ODESolverTypes::clear_ODESolverType_Rythmos
      PROCEDURE,PASS :: clear => clear_ODESolverType_Rythmos
      !> @copybrief ODESolverTypes::step_ODESolverType_Rythmos
      !> @copydetails ODESolverTypes::step_ODESolverType_Rythmos
      PROCEDURE,PASS :: step => step_ODESolverType_Rythmos
  ENDTYPE ODESolverType_Rythmos
...
!>
!> This routine initializes the data spaces for the Rythmos ODE solver interface.
!>
    SUBROUTINE init_ODESolverType_Rythmos(solver,Params,f)
      CHARACTER(LEN=*),PARAMETER :: myName='init_ODESolverType_Rythmos'
      CLASS(ODESolverType_Rythmos),INTENT(INOUT) :: solver
      TYPE(ParamType),INTENT(IN) :: Params
      CLASS(ODESolverInterface_Base),POINTER :: f
      ...
      IF(.NOT. solver%isInit) THEN
        ...
        IF(solvetype == THETA_METHOD) THEN
              CALL eODESolverType%raiseError('Incorrect input to '// &
                modName//'::'//myName//' - Theta method is not supported with Rythmos')
        ...
        solver%TPLType=ODE_RYTHMOS

        ALLOCATE(solver%ytmp0(solver%n))
        ALLOCATE(solver%ytmpf(solver%n))
#ifdef FUTILITY_HAVE_Trilinos
        CALL TS_Init(solver%id,C_FUNLOC(Rythmos_eval),solver%n,solver%tol)
        EXTERNAL_isInit=.TRUE.
        solver%isInit=.TRUE.
#else
        CALL eODESolverType%raiseError('Error in '// &
          modName//'::'//myName//' - Rythmos interface is not available')
#endif
      ELSE
        CALL eODESolverType%raiseError('Incorrect call to '// &
          modName//'::'//myName//' - ODESolverType already initialized')
      ENDIF
    ENDSUBROUTINE init_ODESolverType_Rythmos
!
!-------------------------------------------------------------------------------
!> @brief Clears the Rythmos ODE Solver Type
!> @param solver The ode solver to act on
!>
!> This routine clears the data spaces
!>
    SUBROUTINE clear_ODESolverType_Rythmos(solver)
      CLASS(ODESolverType_Rythmos),INTENT(INOUT) :: solver
      ...
      solver%BDForder=5
      IF(ALLOCATED(solver%ytmp0)) DEALLOCATE(solver%ytmp0)
      IF(ALLOCATED(solver%ytmpf)) DEALLOCATE(solver%ytmpf)
#ifdef FUTILITY_HAVE_Trilinos
      IF(EXTERNAL_isInit) CALL TS_Destroy(solver%id)
#endif
     solver%id=-1
      ...
      solver%isInit=.FALSE.
    ENDSUBROUTINE clear_ODESolverType_Rythmos
!
!-------------------------------------------------------------------------------
!> @brief Performs time step for the Rythmos ODE Solver Type
!> @param solver The ode solver to act on
...
!>
!> This routine performs a solve using the Rythmos solver
!>(
    SUBROUTINE step_ODESolverType_Rythmos(solver,t0,y0,tf,yf)
      CLASS(ODESolverType_Rythmos),INTENT(INOUT) :: solver
      REAL(SRK),INTENT(IN) :: t0
      CLASS(VectorType),INTENT(INOUT) :: y0
      REAL(SRK),INTENT(IN) :: tf
      ...
      CALL y0%get(solver%ytmp0)
#ifdef FUTILITY_HAVE_Trilinos
      CALL TS_Step(solver%id,t0,tf,solver%ytmp0,solver%ytmpf)
#endif
     CALL yf%set(solver%ytmpf)
    ENDSUBROUTINE step_ODESolverType_Rythmos
!
!-------------------------------------------------------------------------------
!> @brief Initializes the ODE Solver Type with a parameter list
!>
