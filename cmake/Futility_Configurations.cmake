...
IF(${PROJECT_NAME}_VERBOSE_CONFIGURE)
    ...
    PRINT_VAR(${PACKAGE_NAME}_ENABLE_CUDA)
    PRINT_VAR(${PACKAGE_NAME}_ENABLE_Kokkos)
...
# Preprocessor to enable valgrind memory checking in tests
IF(Futility_ENABLE_MEMORYCHECK)
    MESSAGE(STATUS "Enabling memory checking for tests")
    SET(CTEST_DO_MEMORY_TESTING TRUE)
    SET(ValgrindCommand "/usr/bin/valgrind" CACHE FILEPATH "")
    SET(ValgrindCommandOptions "--trace-children=yes --tool=memcheck --leak-check=full --show-reachable=no" CACHE STRING "")
    SET(CTEST_MEMORYCHECK_COMMAND "/usr/bin/valgrind" CACHE FILEPATH "")
    SET(CTEST_MEMORYCHECK_COMMAND_OPTIONS "--trace-children=yes --tool=memcheck --leak-check=full --show-reachable=no" CACHE STRING "")
    SET(CTEST_MEMORYCHECK_SUPPRESSIONS_FILE "" CACHE FILEPATH "")
ENDIF()
MESSAGE(STATUS "Memory options:")
MESSAGE(STATUS "${CTEST_MEMORYCHECK_COMMAND} ${CTEST_MEMORYCHECK_COMMAND_OPTIONS} ${CTEST_MEMORYCHECK_SUPPRESSIONS_FILE}")
